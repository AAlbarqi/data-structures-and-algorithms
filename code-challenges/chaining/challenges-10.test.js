...
Build a simple express server. Connect a '/hello' route that sends a greeting of your  choice. Connect a '/aboutme'
 route that sends a short bio about you to the front-end. Finally, connect a '/favoritefoods' route that sends an
  array to the front-end of your favorite foods. All other routes should respond with a status of 404.
------------------------------------------------------------------------------------------------ */
...
const createServer = () => {
  ...
  app.get('/favoritefoods', (req,res) =>{
    var favFood = ['pizza','cheesecake', 'kebab', 'pasta'];
    res.send(favFood);
  });
  app.all('*', (req, res) => {
...
const divisibleByFiveTwoToThePower = (input) => {
  let divisByFive = input.map(val => {
    return val.filter(value => {
      return typeof value === 'number' && value % 5 === 0;
    })
  })
  return divisByFive.map(val => {
